...
namespace Decoder
{
    public partial class FormatterSettingsForm : Form
    {
        ...
        public FormatterSettingsForm(RecordFormatterSettings settings)
        {
            ...
            defSchemaComboBox.Items.AddRange(D900CdrDefinitionProvider.Instance.AvailableSchemas());
            defSchemaComboBox.SelectedItem = D900CdrDefinitionProvider.Instance.CurrentSchema;
        ...
        private void ShowSchemaElements()
        {
            ...
            D900CdrDefinitionSchema schema = D900CdrDefinitionProvider.Instance.GetSchema((string)defSchemaComboBox.SelectedItem);

            ListViewItem item;

            foreach (D900CdrElementDefinition def in schema.Definitions.Values)
            {
        ...
        private void addColButton_Click(object sender, EventArgs e)
        {
            if (elementsListView.SelectedIndices.Count > 0)
            {
                ...
                D900CdrElementDefinition def = new D900CdrElementDefinition(
                            item.Text,
